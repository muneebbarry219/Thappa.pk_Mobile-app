...
export default function VerifyOtpScreen() {
  const { phone, name, verificationId } = useLocalSearchParams<{ phone: string; name?: string; verificationId?: string }>(); const [otp, setOtp] = useState(""); const [loading, setLoading] = useState(false); const { login } = useAuth(); const Alert = useAlertPrompt(); const otpLength = verificationId ? 6 : 4;
